...
parser = argparse.ArgumentParser(description="IBILINEAR microkernel test generator")
parser.add_argument(
    "-s",
    "--spec",
    metavar="FILE",
...
def split_ukernel_name(name):
    match = re.fullmatch(
        r"xnn_(f16|f32)_ibilinear_chw_ukernel__(.+?)(?:_[pu](\d+)v*)?", name
    )
    assert match is not None
    pixel_tile = int(match.group(3)) if match.group(3) else 1
    channel_tile = 1

...
static void f32_ibilinear_chw(benchmark::State& state,
  ...
  xnnpack::Buffer<float> input((channels - 1) * input_stride +
                               4 * output_pixels);
  xnnpack::Buffer<float, XNN_ALLOCATION_ALIGNMENT> packed_weights(output_pixels *
                                                                  2);
...
static void BilinearArguments(benchmark::internal::Benchmark* b) {
...
BENCHMARK_IBILINEAR_CHW(xnn_f32_ibilinear_chw_ukernel__scalar_p1, 0);
